...
`--live` requires a private `--catalog` or `--directory` and refuses this folder. `--catalog` unions LastRound / MIT JSON / CDX snapshots, optionally scouts HN/YC/startups/Speedrun with public GETs, then samples by provider share to `max_boards`. Keep real CSVs and tokens in ignored `private-data/`. Attribute LastRound (CC BY 4.0) when that CSV is used.
