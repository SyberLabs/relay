...
void test('wrong release SHA and workspace access through the service token do not retry', async () => {
  let calls = 0;
  const fetchImpl = async (url) => {
    calls += 1;
    const path = new URL(url).pathname;
    if (path === '/healthz') return jsonResponse(200, { status: 'ok', release: 'b'.repeat(40) });
    return new Response('', { status: 200 });
  };
  await assert.rejects(
    () => smokeRelease({ env, fetchImpl, sleep: async () => assert.fail('must not sleep'), now: () => 0 }),
    /\/healthz did not return the expected release/,
  );
  assert.equal(calls, 1);

  calls = 0;
  const isolation = async (url) => {
    calls += 1;
    const path = new URL(url).pathname;
  ...
  await assert.rejects(
    () => smokeRelease({ env, fetchImpl: isolation, sleep: async () => assert.fail('must not sleep'), now: () => 0 }),
