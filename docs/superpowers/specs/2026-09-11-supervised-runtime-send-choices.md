...
| **T2** | Unpacked Chrome MV3 operative runs the Inspect handshake on a **fictional fixture**.                                                                                                           | [#177](https://github.com/SyberLabs/relay/issues/177) / [#179](https://github.com/SyberLabs/relay/pull/179)    | **Open.** CI green on `3e0b152`. **Not mergeable:** Seth **REQUEST_CHANGES** until all four live findings on that head are resolved: (1) `permissions.request` must be the first async on Start (grant HTTPS Relay origin, then `loadJobs`); (2) an error after Submit must stay **uncertain**, not `not-submitted`; (3) a failed terminal `complete`/`uncertain` POST must not be reported as saved; (4) a pre-submit heading must not count as a fixture receipt. |
...
**Immediate next (T2, still #177):** resolve every live #179 finding on `3e0b152` (grant-then-load, post-Submit uncertainty, terminal record-write failures, and fixture confirmation vs pre-submit heading), then Seth re-reviews. Do not start T3–T6 on that PR.
...
| ---------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- | ---------------------------------------------------------------- |
| Merge T2 after every live #179 finding                                                                     | Seth (approve); Mateo (implement the four blockers)                                                     | Indirect (T3 assumes a first-party operative exists) | Yes, as the hands; still fixture until an adapter exists         |
...
| T6: Runtime + Tracker are the real product, then delete `/plan` `/preferences` `/review` as primary routes | Seth                                                                                                    | **Yes** (this _is_ T6)                               | No                                                               |
...
- Implementing Seth’s remaining T2 blockers on #179 (grant-then-load, post-Submit uncertainty, terminal record writes, fixture confirmation).
...
- [#177](https://github.com/SyberLabs/relay/issues/177) / [PR #179](https://github.com/SyberLabs/relay/pull/179) (grant-then-load, post-Submit uncertainty, terminal record writes, and fixture confirmation REQUEST_CHANGES)
