...
## Profile, preferences, review, and track session expiry (issue 53)

A 401 on those mounted pages expires the page session before parsing the body, clears private state and edit controls, and ignores older in-flight reads or mutations. The existing signed-out screen is shown; same-mount reauthentication is not offered. Delayed restoration was not observed in prior QA; `tests/page-session.test.mjs` covers that race plus GET 401 and the review sibling GET, and `tests/e2e/page-expiry.spec.ts` covers same-screen POST 401 (JSON and plaintext) plus profile extract 401. Workspace expiry is unchanged.
