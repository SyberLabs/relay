...
const env = {
  ...
  DEPLOY_URL: 'https://staging.relay.example',
};
...
void test('production configuration cannot expose static assets before identity verification', () => {
  for (const extra of [{}, { WORKERS_DEV_SUBDOMAIN: workersLabel }]) {
    ...
    assert.deepEqual(config.routes, [
      { pattern: 'staging.relay.example', custom_domain: true },
    ]);
...
void test('configuration refuses absent credentials, placeholder database, invalid names, and non-origin URLs', () => {
  for (const name of Object.keys(env))
    assert.throws(() => releaseConfig({ ...env, [name]: '' }, {}));
  assert.throws(() =>
    releaseConfig(
      { ...env, D1_DATABASE_ID: '00000000-0000-4000-8000-000000000000' },
      {},
    ),
  );
  for (const url of [
...
void test('workers.dev targets fail closed without an exact opted-in origin', () => {
  ...
  const cases = [
    ...
    {
      ...ok,
      DEPLOY_URL: `https://relay-production.${workersLabel}.workers.dev`,
    },
    {
      ...ok,
      DEPLOY_URL: `https://preview.relay-staging.${workersLabel}.workers.dev`,
    },
    {
      ...ok,
      DEPLOY_URL: `https://relay-staging.${workersLabel}.extra.workers.dev`,
    },
...
void test('workers.dev subdomain labels accept 1 and 63 character DNS labels and reject malformed labels', () => {
  ...
  for (const subdomain of [
    ...
    assert.equal(
      new URL(input.DEPLOY_URL).hostname,
      `relay-staging.${subdomain}.workers.dev`,
    );
...
void test('artifact verification binds release SHA and every byte, rejecting missing and extra files', () => {
  ...
  assert.throws(() =>
    verifyManifest(manifest, env.RELEASE_SHA, {
      ...files,
      'dist/extra.js': 'e',
    }),
  );
  assert.throws(() =>
    verifyManifest(manifest, env.RELEASE_SHA, {
      ...files,
      'dist/server/index.js': 'tampered',
    }),
  );
  assert.throws(() =>
    verifyManifest({ ...manifest, files: {} }, env.RELEASE_SHA, {}),
  );
});
