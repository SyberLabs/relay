...
## T-stage status (2026-09-16)

Program home: [#168](https://github.com/SyberLabs/relay/issues/168). Children exist for T0–T6. T6 is the last line of **this** program and is not a named ATS adapter.
...
| **T4** | Split remaining `app/workspace.tsx` into a Runtime hook plus views. Same behavior, no new surface.                                                                                             | [#195](https://github.com/SyberLabs/relay/issues/195) / [#198](https://github.com/SyberLabs/relay/pull/198)    | **Issued.** Draft PR vs `main`. Peer merge pending.                                                                                                                                                                                                                                                                                   |
| **T5** | Collapse plant copy onto the two-screen model: **Runtime** (`/`) and **Tracker** (`/track`). Applications/facts/Advanced are secondary.                                                        | [#196](https://github.com/SyberLabs/relay/issues/196) / [#199](https://github.com/SyberLabs/relay/pull/199)    | **Issued.** Draft PR stacked on T4. Peer merge pending.                                                                                                                                                                                                                                                                               |
| **T6** | Stop using heuristic fact-overlap as pick/gate copy. Fold `/plan`, `/preferences`, and `/review` under Advanced; keep the APIs; bookmarks redirect.                                            | [#197](https://github.com/SyberLabs/relay/issues/197) / [#200](https://github.com/SyberLabs/relay/pull/200)    | **Issued.** Draft PR stacked on T5. T6 is not “we apply to Greenhouse.” Peer merge pending.                                                                                                                                                                           |

**Immediate next:** peer-merge T4 → T5 → T6 in order. Do not self-approve. Do not enable `live`. Do not Worker-POST ATS. Do not promote production. Finishing T6 does not close #168’s product-market-fit question ([#5](https://github.com/SyberLabs/relay/issues/5)) and does not ship a named ATS adapter.

**After T6 merge:** remaining send-path work is a named adapter (host, fields, submit control) still under A, B, Inspect Accept, and one Submit — outside T3–T6. Windows App Control vs Playwright persistent Chromium is [#194](https://github.com/SyberLabs/relay/issues/194). Broader one-step send is [#174](https://github.com/SyberLabs/relay/issues/174).
...
| T4 split `workspace.tsx`                                                                                   | Seth (when); Mateo                                                                                      | **Yes** (sequence says so); #195 / #198              | No                                                               |
| T5 vocabulary → Runtime + Tracker                                                                          | Seth                                                                                                    | **Yes**; #196 / #199                                 | No                                                               |
| T6: Runtime + Tracker are the real product, then delete `/plan` `/preferences` `/review` as primary routes | Seth                                                                                                    | **Yes** (this _is_ T6); #197 / #200              | No                                                               |
...
- Re-opening merged T2 #179 findings as if they were still blocking T3–T6.
- Treating T6 as a Greenhouse/Lever adapter, Worker POST, Durable Object, Browser Rendering, or paid models.
- Changing `docs/public-copy.json` for this fold (public capabilities are unchanged).
- Enabling `live`, promoting production, or closing #5 / #174 / #194.
