...
test('an unknown attribute scores neutral, never zero', () => {
...
test('compensation is scored on a log scale within its band', () => {
...
test('fitted weights recover a preference the choices actually express', () => {
...
test('weights are scaled to unit mass so attributes read as shares', () => {
...
test('no choices yields no opinion rather than a fabricated one', () => {
  assert.deepEqual(Object.values(fitWeights([])), [0, 0, 0, 0, 0]);
});

test('elicitation avoids pairs already asked and one-attribute pairs', () => {
...
test('identical postings offer no informative comparison', () => {
  const corpus = corpusOf(['engineer']);
  const same = [posting({ job_key: 'a' }), posting({ job_key: 'b' })];
  assert.equal(nextPair(same, fitWeights([]), corpus), null);
});

test('a set with no spread normalises to a constant, not invented range', () => {
...
test('expected maximum matches the hand computation', () => {
...
test('order of the input does not change the expected maximum', () => {
  const a = [
    { u: 0.2, p: 0.9 },
    { u: 0.9, p: 0.1 },
    { u: 0.5, p: 0.4 },
...
test('a duplicate of what you already hold adds almost nothing', () => {
...
test('selection respects the attention budget and prefers gain per minute', () => {
  const candidates = [
...
test('a portfolio mixes odds without a hardcoded tier ratio', () => {
...
test('an item costing more than the whole budget is never selected', () => {
  const plan = selectPortfolio([{ job_key: 'x', u: 1, p: 1, effort: 500 }], 60);
  assert.deepEqual(plan.chosen, []);
  assert.equal(plan.spent, 0);
});

test('response rate is a prior until evidence arrives, and reports its width', () => {
...
test('a stale posting is discounted and a dateless one assumed middling', () => {
...
test('tiers are cut on probability', () => {
...
test('compensation is parsed from the prose boards actually publish', () => {
  assert.deepEqual(
...
test('level and remote are inferred from the words employers use', () => {
...
test('markup is stripped without swallowing the text', () => {
  assert.equal(
    stripHtml(
...
test('effort rises with length and supplemental questions', () => {
...
test('a Greenhouse board normalises onto the workspace record shape', () => {
...
test('a Lever board normalises to the same shape', () => {
...
test('a malformed board payload is reported, not silently emptied', () => {
...
test('terminal states are local only and never valid for import', () => {
...
test('rediscovery cannot resurrect a job that already ended', () => {
  for (const terminal of ['Offer', 'Accepted', 'Closed'])
...
test('the existing five-status merge behaviour is unchanged', () => {
...
test('a submission cannot be recorded without a receipt', () => {
  assert.throws(
...
test('outcomes must follow the process they describe', () => {
  assert.throws(
    () =>
      validateOutcome(
        { status: 'Held' },
...
test('each outcome maps to the status it actually implies', () => {
...
test('only receipted submissions count as evidence', () => {
  const rows = [
...
test('a job that has ended stays editable for notes but not for status', () => {
  assert.doesNotThrow(() =>
    validateEdit(
