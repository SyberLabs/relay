...
- Supply-chain compromise: frozen dependency lockfile; pinned Actions; dependency advisories and CodeQL; restricted workflow token; no deployment secrets in pull request jobs. The sykosyber implementation workflow may use `CURSOR_API_KEY` and cannot inherit Cloudflare or Access secrets. Issue bodies are untrusted prompt input, not extra authority.
