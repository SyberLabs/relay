...
3. Call `relay_wait_for_application` with the pinned job, `preparation_revision`, operation `id`, `digest`, and `actor`. Default `wait_ms` is 40000 for wait-only. When this same Cursor MCP invocation must also begin, Submit once, and record the receipt, pass `wait_ms: 20000` so the combined work stays under the host’s ~60-second tool deadline. Leave that invocation pending. The human clicks **Accept and send** once on the signed-in workspace. The tool returns the authorized operation to this still-running host. It cannot wake a terminated host. Five minutes is not supported on Cursor MCP. A clean timeout may be followed by a new explicit wait. Chat “yes” and draft Ready are not send permission. Do not poll `relay_inspect_application` yourself while this wait is running, and do not start a second wait in the same tab.
...
### One invocation through send

Preplan employer fill and Submit before calling wait. After Accept, the same still-running host must `begin`, require `execute === true`, compare the frozen manifest to the prepared form, Submit once, observe confirmation, and finish with that observed receipt. Do not return to the user for another prompt and do not start a later model turn for those steps. Separate roundtrips after wait caused delay and truthful `uncertain` results; keep those uncertain, and do not retry.

A terminated host cannot be woken. Cursor Grok 4.6 Extra High, local fictional HTTPS fixture: one MCP `browser_run_code` invocation completed that path; the fixture observed exactly one POST and recorded Submitted 1.209 seconds after authorization reached the host. That approval was root-automated fixture Accept, not human Inspect in staging or production.
...
Remaining manual boundaries: open/sign into Relay; create the intended job and confirm missing facts; keep a compatible browser tab available with the wait invocation still running through Accept, begin, one Submit and receipt; review and explicitly accept wording; resolve expired authentication, CAPTCHA or quotas before deliberately retrying refused work. `relay_wait_for_application` cannot resume a terminated host. Local Cursor Grok 4.6 Extra High fixture proof does not establish human Inspect, staging, production, or other assistants. Approval through chat, automatic saved-fact relevance selection, background generation and sending are absent.
