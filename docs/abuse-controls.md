...
Inspect presence (`POST /api/applications` `action=arm`, #135) is not a mutation: work weight 1 and 6 arm requests per verified user per minute. The operative refreshes the arm about every 10 seconds; Accept is enabled only while `armed_until` is in the future (20-second window). Prepare remains mutation weight 10. Approve authorizes the frozen payload only while that arm is live and does not begin execution.
