...
test('dirty compares draft and blocker against the loaded base', () => {
...
test('same-job reselect keeps the editor; another job does not', () => {
  const editor = loadEditor(job('A'));
...
test('exact local acceptance is Ready with an unchanged draft and blocker', () => {
  const ready = {
    id: 'A',
    version: 1,
    status: 'Ready',
...
test('queue hint drops accepted copy when the selected Ready editor is dirty', () => {
  const ready = {
    ...job('A', { draft: 'Exact accepted', blocker: '' }),
    status: 'Ready',
    accepted_draft: 'Exact accepted',
...
test('reverting after a Ready conflict does not label stale text accepted', () => {
