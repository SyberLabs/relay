...
3. A capable host keeps `relay_wait_for_application` pending on that exact pin (viewer, job, preparation revision, operation id/digest). Preplan fill and Submit so the same invocation can continue through begin, one Submit, and observed receipt. Default `wait_ms` is 40000 for wait-only; pass `wait_ms: 20000` when Cursor MCP must finish that combined work under its ~60-second tool deadline. The helper ceiling is five minutes and 126 requests, one wait per tab. A clean `wait_timeout` may be followed by a new explicit wait; refusals are not auto-retried. It cannot wake a terminated host. Five minutes is not supported on Cursor MCP. **Accept and send** is enabled only while the payload is complete and `armed_until` is in the future. Clicking it approves that digest (`authorized`). Draft Ready, chat “yes”, and policy `review: sensitive` ordinary-field shortcuts are not send permission on Inspect.
