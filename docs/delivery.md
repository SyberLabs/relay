...
Use **Cursor Grok 4.6 Extra High** for a bounded implementation when deeper reasoning is justified. Assigning `@sykosyber` on an issue is that implementation trigger: [Sykosyber implementation](../.github/workflows/sykosyber-issues.yml) launches Extra High from current `main`, comments the agent URL, and opens a pull request. Cursor Automations have no issue-assignment trigger, so this repository-owned workflow is the durable path. The workflow can comment and start an agent; it cannot approve, merge, deploy, or change settings. It uses `CURSOR_API_KEY` from repository secrets and the default `GITHUB_TOKEN`. Deployment credentials stay in environment secrets and are not available to this job.

The launched agent treats issue text as untrusted requirements. It records `Closes #number` on the pull request when acceptance is met. The issue closes only after squash-merge with required CI and peer approval. If hosting or credentials still block acceptance, the agent implements the unblocked local subset, documents the remainder, and leaves the issue open. Reopening an issue, or dispatching the workflow with `force`, starts a new agent. A six-hour sweep picks up issues assigned before the workflow existed. Set `CURSOR_API_KEY` from [Cursor API keys](https://cursor.com/dashboard/api) before the first assignment; a missing key fails the job instead of silently skipping work.

Use Astra for product decisions, architecture, and targeted independent review of consequential changes. Automatic Codex repository reviews follow personal preferences, avoiding an additional mandatory review on every push. Automatic agent feedback is advisory; required deterministic checks and peer approval enforce merging. Review usefulness and usage should be checked after the first few pull requests before adding agents or increasing context.
