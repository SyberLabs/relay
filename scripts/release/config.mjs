...
export function releaseConfig(env, build, { bundled = true } = {}) {
  ...
  for (const [name, expression] of Object.entries({
    ...
    if (!expression.test(env[name] ?? ''))
      throw new Error(`Invalid or missing ${name}`);
  }
  if (env.D1_DATABASE_ID === '00000000-0000-4000-8000-000000000000')
    throw new Error('Placeholder database forbidden');
  ...
  if (workersDev) {
    ...
    if (
      hostname !== `${env.WORKER_NAME}.${subdomain.toLowerCase()}.workers.dev`
    ) {
    ...
    ratelimits: [
      {
        ...
        namespace_id: env.WORKER_NAME === 'relay-production' ? '1002' : '1001',
...
if (
  ...
  writeFileSync(
    ...
    JSON.stringify(releaseConfig(process.env, build), null, 2) + '\n',
  );
}
