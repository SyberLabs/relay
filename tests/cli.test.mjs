...
import { parseArgs } from '../integrations/cli.mjs';

test('arguments parse into positionals and flags in every form', () => {
...
test('an inline value keeps characters that would otherwise split it', () => {
...
test('domain refusals and server failures map to different exit codes', () => {
...
test('the CLI refuses to point at anything but a local server', () => {
...
test('an unreachable server is a retryable failure, not a refusal', async () => {
...
test('a refusal carries the server message and the refused code', async () => {
  await assert.rejects(
    () =>
      request(
        '/api/drafts',
...
test('a non-JSON body is reported rather than parsed into nonsense', async () => {
  await assert.rejects(
    () =>
      request('/api/plan', undefined, {
        session: 'x=1',
...
test('requests without a session stop before reaching the network', async () => {
  let called = false;
  await assert.rejects(
    () =>
      request('/api/plan', undefined, {
...
test('the session cookie travels on every request and nothing else does', async () => {
  let seen;
