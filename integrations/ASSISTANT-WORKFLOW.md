...
3. Call `relay_wait_for_application` with the pinned job, `preparation_revision`, operation `id`, `digest`, and `actor`. Default `wait_ms` is 40000. Leave that invocation pending. The human clicks **Accept and send** once on the signed-in workspace. The tool returns the authorized operation to this still-running host. It cannot wake a terminated host; a helper in source is not proof that Cursor, Grok, or ChatGPT kept the call open. Cursor MCP times out around 60 seconds, so keep `wait_ms` at 40000 or less there; five minutes is not supported on that host. A clean timeout may be followed by a new explicit wait. Chat “yes” and draft Ready are not send permission. Do not poll `relay_inspect_application` yourself while this wait is running, and do not start a second wait in the same tab.
