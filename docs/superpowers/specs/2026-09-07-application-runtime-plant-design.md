...
| Stuck mid-submit                                  | `jobs.blocker` + BlockerReview                                                                   | South lane is jobs with a blocker that are not sent/ended. Clicking selects the job in core (BlockerReview stays usable). **Answer the open question** opens the blocked-answer modal. |
