...
2. Create a short branch such as `feat/12-import-preview` or `fix/18-stale-draft`. Keep at most one active implementation issue per person. Only one pull request is landing on `main`; keep others draft or Blocked with the named landing PR.
3. Make the smallest complete change. Add regression tests for actual behavior and failure paths. Use fictional data.
4. Open a pull request with `Closes #number`, evidence, and migration/recovery impact. Keep it draft unless it is the landing PR; move the issue to In review only then.
...
6. Squash merge after all required checks pass and conversations are resolved. During freeze, do not update from `main` unless the peer asked for a product fix on that SHA. The branch is removed automatically. Production release has a separate approval and smoke verification.
